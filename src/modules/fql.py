...
fql_doc = """Falcon Query Language (FQL)
...
class FqlModule(BaseModule):
    ...
    def register_resources(self, server: FastMCP) -> None:
        ...
        basic_resource = FunctionResource(
            uri=AnyUrl("resource://fql_documentation"),
            name="fql_documentation",
            description="Contains the documentation to build FQL queries.",
            fn=self.fql_documentation,
        )

        self._add_resource(server, basic_resource)

    def fql_documentation(self) -> str:
        return fql_doc
